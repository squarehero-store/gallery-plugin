    ...
    class MasonryGridWidget {
        constructor() {
            ...
            this.layoutOptions = [
                ...
                { id: '60-20-20', name: '50 / 25 / 25', items: 3 },
                { id: '25-25-50-row', name: '25 / 25 / 50', items: 3 },
        ...
        attachEventListeners() {
            ...
            document.addEventListener('mousedown', (e) => {
                ...
                // Toggle switch (full-width, crop-images, or preview-video)
                if (target.closest('.toggle-switch')) {
                    ...
                    const itemIndex = parseInt(toggle.getAttribute('data-item-index'));
                    const toggleType = toggle.getAttribute('data-toggle-type');
                    const isCurrentlyActive = toggle.classList.contains('active');
                    
                    if (toggleType === 'previewVideo') {
                        // Handle preview video toggle
                        if (this.gridData[rowIndex] && this.gridData[rowIndex].items[itemIndex]) {
                            ...
                            if (newState) {
                                ...
                                item.showPreviewUpload = true;
                            } else {
                                // Hide upload area and remove preview video
                    ...
                    } else if (this.gridData[rowIndex]) {
                        // Handle row-level toggles (fullWidth, cropImages)
                        const newState = !isCurrentlyActive;
                        
                        if (toggleType === 'fullWidth') {
                            this.gridData[rowIndex].fullWidth = newState;
                        } else if (toggleType === 'cropImages') {
                            this.gridData[rowIndex].cropImages = newState;
                        }
                ...
                // Preview video select button
                if (target.closest('.preview-video-select-btn')) {
                    e.preventDefault();
                    e.stopPropagation();
                    const btn = target.closest('.preview-video-select-btn');
                    const rowIndex = parseInt(btn.getAttribute('data-row'));
                    const itemIndex = parseInt(btn.getAttribute('data-item'));
                    
                    if (this.gridData[rowIndex] && this.gridData[rowIndex].items[itemIndex]) {
                        // Set flag to indicate preview video mode
                        this.uploadingPreviewVideo = { rowIndex, itemIndex };
                        // Use the same library selector as main video
                        this.selectFromLibrary(rowIndex, itemIndex);
                    }
                    return false;
                }
                
                // Preview video remove button
                if (target.closest('.preview-video-remove')) {
                    e.preventDefault();
                    e.stopPropagation();
                    const btn = target.closest('.preview-video-remove');
                    const rowIndex = parseInt(btn.getAttribute('data-row'));
                    const itemIndex = parseInt(btn.getAttribute('data-item'));
                    
                    if (this.gridData[rowIndex] && this.gridData[rowIndex].items[itemIndex]) {
                        delete this.gridData[rowIndex].items[itemIndex].previewVideo;
                        this.renderAdminForm();
                        this.markAsChanged();
                    }
                    return false;
        ...
        createLayoutBuilder() {
            ...
            const layouts = [
                ...
                { id: '67-left', name: '2/3 Left Aligned', preview: this.createLayoutPreview('67-left', 'builder') },
                { id: '67-right', name: '2/3 Right Aligned', preview: this.createLayoutPreview('67-right', 'builder') },
                ...
                { id: '25-25-50-row', name: '25 / 25 / 50', preview: this.createLayoutPreview('25-25-50-row', 'builder') },
        ...
        addRowWithLayout(layoutId) {
            ...
            const newRow = {
                ...
                cropImages: false // Add crop images property with default value (no crop by default)
            };
        ...
        createLayoutPreview(layoutId, context = 'default') {
            ...
            if (context === 'builder') {
                ...
                const builderLayoutMap = {
                    ...
                    '60-20-20': `<div class="${itemClass}" style="flex: 2;"></div><div class="${itemClass}" style="flex: 1;"></div><div class="${itemClass}" style="flex: 1;"></div>`,
                    '25-25-50-row': `<div class="${itemClass}" style="flex: 1;"></div><div class="${itemClass}" style="flex: 1;"></div><div class="${itemClass}" style="flex: 2;"></div>`,
                ...
                const accordionLayoutMap = {
                    ...
                    '60-20-20': `<div class="${itemClass}" style="flex: 2; height: 20px;"></div><div class="${itemClass}" style="flex: 1; height: 20px;"></div><div class="${itemClass}" style="flex: 1; height: 20px;"></div>`,
                    '25-25-50-row': `<div class="${itemClass}" style="flex: 1; height: 20px;"></div><div class="${itemClass}" style="flex: 1; height: 20px;"></div><div class="${itemClass}" style="flex: 2; height: 20px;"></div>`,
        ...
        createRowForm(row, rowIndex) {
            ...
            fullWidthGroup.innerHTML = `
                ...
                <div class="full-width-toggle-container">
                    <div class="toggle-switch ${row.fullWidth ? 'active' : ''}" data-row-index="${rowIndex}" data-toggle-type="fullWidth">
            ...
            // Create second row of controls for image fit
            const bottomControls2 = document.createElement('div');
            bottomControls2.className = 'row-bottom-controls';
            bottomControls2.style.display = 'flex';
            bottomControls2.style.gap = '15px';
            bottomControls2.style.alignItems = 'flex-end';
            bottomControls2.style.marginTop = '15px';
            ...
            cropImagesGroup.innerHTML = `
                ...
                <div class="crop-images-toggle-container">
                    <div class="toggle-switch ${cropImages ? 'active' : ''}" data-row-index="${rowIndex}" data-toggle-type="cropImages">
                        <div class="toggle-switch-slider"></div>
            ...
            bottomControls2.appendChild(cropImagesGroup);
            form.appendChild(bottomControls2);
        ...
        createLayoutAreaHTML(row, rowIndex, itemIndex, defaultContent = null) {
            ...
            const hasPreviewVideo = isVideo && item.previewVideo;
            
            return `
                <div class="layout-area-wrapper">
                    <div class="layout-area" draggable="true" data-row-index="${rowIndex}" data-item-index="${itemIndex}" data-drag-row-index="${rowIndex}">
                    ...
                    ${isVideo ? `
                        <div class="preview-video-controls">
                            <div class="toggle-switch ${hasPreviewVideo || item.showPreviewUpload ? 'active' : ''}" 
                            ...
                            ${hasPreviewVideo || item.showPreviewUpload ? `
                                <div class="preview-video-upload-area" data-row="${rowIndex}" data-item="${itemIndex}">
                                    ${item.previewVideo ? this.getPreviewVideoThumbnail(item.previewVideo, rowIndex, itemIndex) : `
                                        <button class="preview-video-select-btn" data-row="${rowIndex}" data-item="${itemIndex}">
        ...
        getPreviewVideoThumbnail(videoUrl, rowIndex, itemIndex) {
            const previewId = `preview-video-${rowIndex}-${itemIndex}`;
            
            // Load thumbnail asynchronously
            setTimeout(async () => {
                const fakeAssetData = { assetUrl: videoUrl };
                const thumbnailUrl = await this.getVideoThumbnail(fakeAssetData);
                ...
                if (element) {
                    if (thumbnailUrl) {
                        ...
                        element.style.backgroundSize = 'cover';
                        element.style.backgroundPosition = 'center';
                        element.innerHTML = ''; // Clear the loading text
                    } else {
                        element.style.background = '#666';
                        element.innerHTML = '<div style="color: white; font-size: 11px;">No thumbnail</div>';
                    }
                }
            }, 100);
            ...
            return `
                <div class="preview-video-thumbnail">
                    <div id="${previewId}" style="width: 100%; height: 100%; background: #f0f0f0; display: flex; align-items: center; justify-content: center; border-radius: 4px;">
                        <span style="font-size: 11px; color: #999;">Loading...</span>
                    </div>
                    <button class="preview-video-remove" data-row="${rowIndex}" data-item="${itemIndex}">×</button>
                </div>
            `;
        }
        ...
        uploadPreviewVideo(rowIndex, itemIndex) {
            console.log('🎬 Starting preview video upload for:', rowIndex, itemIndex);
            
            // Create and trigger file input IMMEDIATELY while event is still "trusted"
            const fileInput = document.createElement('input');
            fileInput.type = 'file';
            fileInput.accept = 'video/*'; // Only accept videos for preview
            fileInput.style.display = 'none';
            ...
            fileInput.onchange = async (inputEvent) => {
                ...
                console.log(`🎬 Selected preview video: ${file.name}`);
                
                try {
                    // Show loading state
                    const loadingOverlay = this.createLoadingOverlay('Uploading preview video...');
                    document.body.appendChild(loadingOverlay);
                    ...
                    // Get asset authorization for video playback
                    if (!this.assetAuthToken) {
                        const assetAuthData = await this.getAssetAuth(crumbToken);
                        if (assetAuthData && assetAuthData.token) {
                            this.assetAuthToken = assetAuthData.token;
                            console.log('🎬 Got asset authorization token for preview video');
                        }
                    }
                    ...
                    const jobId = await this.uploadRawMediaFile(file, libraryId);
                    if (!jobId) {
                        throw new Error('Failed to upload raw video file.');
                    }
                    
                    const assetId = await this.pollForAssetId(jobId, true);
                    if (!assetId) {
                        throw new Error('Failed to get asset ID after upload.');
                    }
                    
                    // For videos, construct the asset info directly
                    console.log('Preview video upload complete! Constructing asset info...');
                    const assetInfo = {
                        id: assetId,
                        assetUrl: `https://video.squarespace-cdn.com/content/v1/${libraryId}/${assetId}/{variant}`
                    };
                    
                    // Activate the newly uploaded video
                    console.log('🎬 📚 Activating preview video via lesson service...');
                    try {
                        await this.performFauxSectionSave(assetId, crumbToken, file.name);
                    } catch (error) {
                        console.warn('🎬 ⚠️  Preview video activation failed, but upload was successful:', error);
                    }
                    
                    // Update the grid item's previewVideo property
                    if (this.gridData[rowIndex] && this.gridData[rowIndex].items[itemIndex]) {
                        this.gridData[rowIndex].items[itemIndex].previewVideo = assetInfo.assetUrl;
                    }
                    ...
                    // Restore accordion states and scroll position
                    setTimeout(() => {
                        this.restoreDetailedAccordionStates(accordionStates);
                        const adminColumn = document.querySelector('.admin-left-column');
                        if (adminColumn) {
                    ...
                    console.log('Preview video uploaded successfully!', assetInfo);
                    
                } catch (error) {
                    console.error('Preview video upload failed:', error);
                    alert(`Preview video upload failed: ${error.message}`);
        ...
        async selectAsset(assetData, rowIndex, itemIndex) {
            ...
            // Check if this is a preview video upload
            const isPreviewUpload = this.uploadingPreviewVideo && 
                                   this.uploadingPreviewVideo.rowIndex === rowIndex && 
                                   this.uploadingPreviewVideo.itemIndex === itemIndex;
            ...
            // If this is a preview video, store in previewVideo property instead
            if (isPreviewUpload) {
                console.log(`📹 Storing as preview video for row ${rowIndex}, item ${itemIndex}`);
                if (this.gridData[rowIndex] && this.gridData[rowIndex].items[itemIndex]) {
                    this.gridData[rowIndex].items[itemIndex].previewVideo = assetUrl;
                    // Keep the upload area visible
                    this.gridData[rowIndex].items[itemIndex].showPreviewUpload = true;
                }
                // Clear the flag
                this.uploadingPreviewVideo = null;
                // Re-render to show the preview thumbnail
                this.renderAdminForm();
                this.markAsChanged();
            } else {
                // Normal asset selection - update the item with correct type and URL
        ...
        updateRowLayout(rowIndex, newLayout) {
            if (this.gridData[rowIndex]) {
                ...
                if (layout) {
                    const newItems = [];
                    const oldLayout = this.gridData[rowIndex].layout;
                    const currentItems = this.gridData[rowIndex].items || [];
                    
                    // Special handling for switching between 67-left and 67-right
                    const switching67Layouts = (oldLayout === '67-left' && newLayout === '67-right') || 
                                               (oldLayout === '67-right' && newLayout === '67-left');
                    
                    if (switching67Layouts) {
                        // When switching between 67-left and 67-right, swap the content and empty positions
                        if (newLayout === '67-left') {
                            // Image should be at position 0, empty at position 1
                            const contentItem = currentItems.find(item => item && item.type !== 'empty');
                            newItems[0] = contentItem || { type: 'placeholder', background: '#667eea', content: 'Item 1' };
                            newItems[1] = { type: 'empty', content: '' };
                        } else {
                            // Image should be at position 1, empty at position 0
                            const contentItem = currentItems.find(item => item && item.type !== 'empty');
                            newItems[0] = { type: 'empty', content: '' };
                            newItems[1] = contentItem || { type: 'placeholder', background: '#667eea', content: 'Item 2' };
                        }
                    } else {
                        // Standard layout change logic
                        for (let i = 0; i < layout.items; i++) {
                            ...
                            if (isEmptySpace) {
                                ...
                                // Try to restore from temporary storage first
                                const restoredImage = this.getStoredImageForPosition(rowIndex, i);
                                if (restoredImage && restoredImage.type !== 'empty') {
                                    newItems[i] = restoredImage;
                                } else {
                                    // Try current item at this position
        ...
        renderGrid() {
            ...
            this.gridData.forEach((row, rowIndex) => {
                ...
                // Add crop/no-crop class based on cropImages setting (default to crop)
                const cropImages = row.cropImages !== undefined ? row.cropImages : true;
                if (cropImages) {
                    rowElement.classList.add('crop-images');
                } else {
                    rowElement.classList.add('no-crop-images');
                }
        ...
        createMasonryItem(item, itemIndex, rowIndex, layout = null) {
            ...
            if (item.type === 'empty') {
                ...
                // Use preview video if available, otherwise use main content
                const videoSource = item.previewVideo || item.content;
                
                // Check if it needs HLS handling (Squarespace videos or m3u8 files)
                const playlistUrl = this.getVideoPlaylistUrl(videoSource);
                const needsHLS = playlistUrl !== videoSource || videoSource.includes('.m3u8');
